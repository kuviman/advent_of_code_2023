...
fn main() {
    ...
    let area = loop_positions
        .iter()
        .copied()
        .circular_tuple_windows()
        .map(|(p1, p2)| match p1.1.cmp(&p2.1) {
            std::cmp::Ordering::Less => -p1.0,
            std::cmp::Ordering::Equal => 0,
            std::cmp::Ordering::Greater => p1.0,
        })
        .sum::<isize>()
        .abs();

    // https://en.wikipedia.org/wiki/Pick%27s_theorem
    let answer: isize = area - loop_positions.len() as isize / 2 + 1;
    println!("{answer}");
}
